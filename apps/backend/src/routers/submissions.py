...
SAFE_OUTBOX_NAME_RE = re.compile(r'^[A-Za-z0-9_.-]+\.eml$')
...
async def outbox_download(name: str):
    if not SAFE_OUTBOX_NAME_RE.fullmatch(name):
        raise HTTPException(400, 'invalid_file_name')

    outbox_root = OUTBOX.resolve()
    requested_path = (OUTBOX / name).resolve()

    # Prevent path traversal outside outbox.
    try:
        requested_path.relative_to(outbox_root)
    except ValueError:
    ...
    return FileResponse(path=requested_path, filename=requested_path.name, media_type='message/rfc822')
