...
def _fallback_bbp_programs() -> List[Dict[str, Any]]:
    if DISCOVERY_AVAILABLE:
        loader, _, _ = _get_discovery_components()
        if loader is not None:
            try:
                programs = loader.load_programs() or []
                if programs:
                    normalized: List[Dict[str, Any]] = []
                    for idx, item in enumerate(programs):
                        row = dict(item)
                        row.setdefault("priority_score", float(row.get("priority_score", 0.0) or 0.0))
                        row.setdefault("name", row.get("program") or row.get("slug") or f"program-{idx + 1}")
...
async def bbp_top50(n: int = 50) -> Dict[str, Any]:
    ...
    if not data:
        data = _fallback_bbp_programs()
        source = "generated_fallback"
    else:
        source = str(CANON_JSONL if CANON_JSONL.exists() else FALLBACK_JSONL)
    data.sort(key=lambda r: r.get('priority_score', 0.0), reverse=True)
    sel = data[: max(1, min(int(n), 50))]
    return {'programs': sel, 'count': len(sel), 'source': source}
        ...
        try:
            ...
            try:
                jwt_token = create_access_token(
                    subject="internal-bbp-queue",
                    roles=[ROLE_ADMIN, ROLE_ANALYST, ROLE_OPERATOR, ROLE_VIEWER],
                )
            except AuthConfigError as auth_err:
                out['auto_run'] = {'status': 'error', 'error': f'jwt_bootstrap_failed: {auth_err}'}
                return out
